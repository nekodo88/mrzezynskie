...
const submit = async (form, submitter) => {
    ...
    const response = await axios.get(`${uri}?${asString}`).catch((err) => {
        console.log(err);
...
const show = () => {
    ...
    if (document.body) {
        ...
        if ((Cookies.get(cookieName) || 0) < age && !bot() && !ag_logged_in) {
...
if (AgeGateHooks) {
    const shouldShow = AgeGateHooks.applyFilters('age_gate_show', true);

    if (shouldShow) {
        setTimeout(show, AgeGateHooks.applyFilters('age_gate_show_timeout', 1));
    }
} else {
    show();
}
