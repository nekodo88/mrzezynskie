...
class Shortcode extends AbstractShortcode
{
                    ...
                    'fields' => collect(explode(' ', $settings->dateFormat))->mapWithKeys(function ($item) use ($map, $settings) {
                        ...
                        return [
                            $key => [
                                ...
                                'options' => $settings->inputType === 'selects' ? $this->getOptions($key) : [],
                            ]
                        ];
                    })->toArray()
                ]
    ...
    private function getOptions($key)
    {
        switch ($key) {
            ...
            case 'y':
                return collect(range(apply_filters('age_gate/form/select/year/min', 1950), 2022))->mapWithKeys(function ($year) {
                    return[$year => $year];
                })->toArray();
        }
    }
