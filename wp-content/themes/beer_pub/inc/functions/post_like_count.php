...
function beer_pub_like_scripts() {
...
add_action('init', 'beer_pub_like_scripts');

add_action('wp_ajax_nopriv_beer_pub-post-like', 'beer_pub_post_like');
add_action('wp_ajax_beer_pub-post-like', 'beer_pub_post_like');

function beer_pub_post_like() {
    $nonce = $_POST['nonce'];
    if (!wp_verify_nonce($nonce, 'ajax-nonce'))
        die('Nope!');

    if (isset($_POST['beer_pub_post_like'])) {
        $like_type = 'post';
        ...
        if (is_user_logged_in()) { // user is logged in
            ...
            if (!beer_pub_AlreadyLiked($post_id)) { // like the post
                if($like_type == 'comment') {
...
function beer_pub_AlreadyLiked($post_id) { // test if user liked before
    $like_type = 'post';
    if (isset($_POST['like_type']) && !empty($_POST['like_type'])) {
        $like_type = $_POST['like_type'];
    }
...
function beer_pub_getPostLikeLink($post_id, $like_type = 'post') {
    if ($like_type == 'comment') {
    ...
    if (beer_pub_AlreadyLiked($post_id)) {
    ...
    $output = '<a href="#" class="beer_pub-post-like' . $class . '"  data-like_type="'.$like_type.'" data-post_id="' . $post_id . '" title="' . $title . '">' . $heart . $count . $like_text .'</a>';
    return $output;
}
